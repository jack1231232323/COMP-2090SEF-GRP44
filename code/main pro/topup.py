...
from UI import style_entry, mk_btn, fade_in, pulse_btn, shake
from config import *
...
class TopUpDialog(tk.Toplevel):
    ...
    def _build(self):
        ...
        quick = tk.Frame(self, bg=BG_ROOT)
        quick.pack(pady=16)
        for amt in [30,50,100,200]:
            mk_btn(quick, f"${amt}", lambda a=amt: self._set_amt(a), bg=BG_ACTIVE, fg=TEXT, width=8).pack(side="left", padx=6)
        ...
        btnf = tk.Frame(self, bg=BG_ROOT)
        btnf.pack(pady=12)
        self.confirm_btn = mk_btn(btnf, "Confirm", self._do_confirm, width=12)
        self.confirm_btn.pack(side="left", padx=8)
        mk_btn(btnf, "Cancel", self.destroy, bg=BG_ACTIVE, width=10).pack(side="left", padx=8)

    def _set_amt(self, amt):
        self.entry.delete(0, tk.END)
        self.entry.insert(0, str(amt))
        self.msg.config(text="")
        pulse_btn(self.confirm_btn)

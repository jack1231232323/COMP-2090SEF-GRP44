...
class TopUpDialog(tk.Toplevel):
    ...
    def do_build(self):
        ...
        tk.Label(self, text=f"Current: ${u.balance:.2f}", font=FONT_BODY, fg=TEXT, bg=BG_ROOT).pack()

        qf = tk.Frame(self, bg=BG_ROOT)
        qf.pack(pady=16)
        for amt in [30, 50, 100, 200]:
            create_button(qf, f"${amt}", lambda a=amt: self.set_amt(a),
        ...
        f.pack(pady=12)
        self.cb = create_button(f, "Confirm", self.do_confirm, width=12)
